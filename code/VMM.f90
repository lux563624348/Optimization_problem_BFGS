...
program main
    ...
    pX0=[1.0,-2.0]!start
    fx=px0(1)**2+2*px0(1)*px0(2)+2*px0(2)**2
...
!该步通过输入坐标X，计算梯度Gx，模mg, 更新Hessian
    use dat1
    implicit none
    real::A,B,C,mg
    integer ::j
...
subroutine dfx(x,df,m)
    !求测试函数导数，偏x 为 df(1),偏y为df(2)
    implicit none
    real::m
    real,dimension(1:2)::x,df
    df(1)=2.0*x(1)+2.0*x(2)
    df(2)=2.0*x(1)+4.0*x(2)
    m=(df(1)**2+df(2)**2)**(0.5)
endsubroutine
